...
export const MemberCard: React.FC<MemberCardProps> = ({
  ...
  const hasMobile = member.mobile && member.mobile !== 'Information not available';
  const cleanDigits = hasMobile ? member.mobile.replace(/\D/g, '').slice(-10) : '';
  ...
  const memberOrg = member.organizationName || (lang === 'en' ? villageSettings.orgName : villageSettings.orgNameHindi);
  const formattedDate = member.createdAt ? member.createdAt.slice(0, 7) : '2026-08';

  return (
    <div className="relative group overflow-hidden rounded-2xl bg-white/80 dark:bg-[#111726]/80 backdrop-blur-md border border-[#E0DCCF]/80 dark:border-slate-800 shadow-xs hover:shadow-xl hover:shadow-emerald-950/5 dark:hover:shadow-emerald-500/5 hover:border-emerald-500/50 dark:hover:border-emerald-500/50 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between p-4 sm:p-5">
      ...
      <div>
        ...
        <div className="flex items-start space-x-3.5 mb-3.5 relative z-10">
          <div className="relative flex-shrink-0">
            <Avatar size="lg" className="w-14 h-14 ring-2 ring-emerald-500/30 dark:ring-emerald-500/40 group-hover:ring-emerald-500/70 transition-all shadow-xs">
              {member.photoUrl ? (
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1 mt-1 text-[11px] text-emerald-800 dark:text-emerald-300 font-semibold truncate">
              <Building2 className="w-3 h-3 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
              <span className="truncate">{memberOrg}</span>
            </div>
            ...
            <p className="text-[11px] text-[#636054] dark:text-slate-300 font-mono mt-1 flex items-center gap-1">
              ...
              <span className="truncate">{hasMobile ? `+91 ${cleanDigits}` : t('members.noPhone')}</span>
            </p>
          </div>
        </div>

        {/* Info Grid Pills */}
        <div className="grid grid-cols-2 gap-1.5 py-2.5 px-3 mb-3.5 bg-[#F8F6F0]/80 dark:bg-[#0B0F17]/60 backdrop-blur-xs rounded-xl border border-[#E0DCCF]/60 dark:border-slate-800/80 text-[10px] text-[#636054] dark:text-slate-300">
          <div className="flex items-center gap-1.5 truncate">
            <Calendar className="w-3 h-3 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
            <span className="truncate">{t('members.joinedDate', { date: formattedDate })}</span>
          </div>

          <div className="flex items-center gap-1.5 truncate">
            <MapPin className="w-3 h-3 text-amber-600 dark:text-amber-400 flex-shrink-0" />
            <span className="truncate">{member.address || (lang === 'en' ? (villageSettings.name || 'Rasoolpur') : villageSettings.nameHindi)}</span>
          </div>
        </div>
